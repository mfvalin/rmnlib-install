...
contains
   ...
   integer function new_build_vert(self,kind,version,nk,ip1,ip2, &
      ...
      integer, dimension(:), pointer :: fptr_int
      real*8, dimension(:), pointer :: fptr_8
      ...
      if(present(b_m_8))then
         fptr_8 => b_m_8
         b_m_8_CP = c_loc(fptr_8(1))
      else
         b_m_8_CP = C_NULL_PTR
      endif
      if(present(a_t_8))then
         fptr_8 => a_t_8
         a_t_8_CP = c_loc(fptr_8(1))
         nl_t = size(a_t_8)
      else
         a_t_8_CP = C_NULL_PTR
      endif
      if(present(b_t_8))then
         fptr_8 => b_t_8
         b_t_8_CP = c_loc(fptr_8(1))
      else
         b_t_8_CP = C_NULL_PTR
      endif
      if(present(ip1_m))then
         fptr_int => ip1_m
         ip1_m_CP = c_loc(fptr_int(1))
      else
         ip1_m_CP = C_NULL_PTR
      endif
      if(present(ip1_t))then
         fptr_int => ip1_t
         ip1_t_CP = c_loc(fptr_int(1))
      else
         ip1_t_CP = C_NULL_PTR
      endif

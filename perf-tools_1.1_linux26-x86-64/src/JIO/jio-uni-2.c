...
/*
   ...
   Revision 002 : M.Valin Environment Canada 10-Dec-2010
          ...
          -refactored code
   Revision 003 : M.Valin UQAM 05-Nov-2013
          open file named JIO_END to produce final report no longer used (used atexit instead)
          added SLOT_... symbols to address statistics tables
*/

#define MAXF 250  /* Default max filenames */
#define AVGL 100  /* Avg length of filenames */
#define NUME 16   /* Number of intercepted I/O routines */
#define MAXD 300  /* Default Max File Desciptors */
...
#define SLOT_fopen 0
#define SLOT_fclose 1
#define SLOT_fread 2
#define SLOT_fwrite 3
#define SLOT_open 4
#define SLOT_close 5
#define SLOT_read 6
#define SLOT_write 7
#define SLOT_fgets 8
#define SLOT_fseek 9
#define SLOT_lseek 10
#define SLOT_fopen64 11
#define SLOT_open64 12
#define SLOT_lseek64 13
#define SLOT_kread 15
#define SLOT_kwrite 15
...
#else
static unsigned long long jio_time2()
{
...
#ifdef linux
static pid_t gettid(void)  /* get thread id not used for now */
{
...
static long long jio_init()
{
  ...
  if(jio_first) {   /* package initialized ? */
  ...
  mpi_child = getenv("OMPI_COMM_WORLD_RANK");
  if(mpi_child != NULL){
    mpi_rank=atoi(mpi_child);
  }
  ...
  if(mpi_rank != -1)  fprintf(stderr,"MPI child %4.4d JIO init\n",mpi_rank);
...
#endif
      jio_add_info(t0,jio_num,SLOT_fopen,fileno(result),0,name);
  }
  return result;
}
...
#endif
    jio_add_info(t0,jio_num,SLOT_fopen64,fileno(result),0,name);
  }
  return result;
}
#endif
...
size_t fread(void *ptr, size_t size, size_t nobj, FILE *stream)
{
  ...
  jio_add_info(t0,jio_num,SLOT_fread,fileno(stream),size*nobj,NULL);
  return result;
}
/* fwrite */
size_t fwrite(const void *ptr, size_t size, size_t nobj, FILE *stream)
{
  ...
  jio_add_info(t0,jio_num,SLOT_fwrite,fileno(stream),size*nobj,NULL);
  return result;
}
/* open, open64 */
...
/*  get mode argument if O_CREAT */
  ...
  if(result > 0) {
    ...
    jio_add_info(t0,jio_num,SLOT_open,result,0,name);
  }
  return result;
}
#ifdef ioroutines64
int open64(const char * name, int flags, ...)
{
  ...
  if(result > 0) {
    ...
    jio_add_info(t0,jio_num,SLOT_open64,result,0,name);
  }
  return result;
}
#endif
...
int close(int fd)
{
  ...
  jio_add_info(t0,jio_num,SLOT_close,fd,0,NULL);
  indx[fd]=0;  /* file is now closed, no entry associated any more with this fd */
...
char *fgets(char *line, int maxline, FILE *fp)
{
  ...
  jio_add_info(t0,jio_num>2?2:jio_num, SLOT_fgets, fileno(fp), maxline, NULL);
...
int fseek(FILE *stream, long offset, int whence)
{
  ...
  jio_add_info(t0,jio_num,SLOT_fseek,fileno(stream),0,NULL);
  return result;
}
...
off64_t lseek64(int fd, off64_t offset, int whence)
{
  ...
  jio_add_info(t0,jio_num,SLOT_lseek64,fd,0,NULL);
  return result;
}
#endif
#ifdef _AIX
...
ssize_t kread(int fd, void * buf, size_t n)
{
  ...
  jio_add_info(t0,jio_num,SLOT_kread,fd,result,NULL);
  return result;
}

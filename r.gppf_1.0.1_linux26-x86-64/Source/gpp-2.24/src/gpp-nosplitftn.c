...
#define MAXARGS 100
#define MAXINCL 100   /* max # of include dirs */
...
struct MODE CUser = {"",            "",   "(",     ",",         ")",     "#",  '\\', "(", ")" };
struct MODE CMeta = {"#",           "\n", "\001",  "\001",      "\n",    "#",  '\\', "(", ")" };
struct MODE FUser = {"",            "",   "(\002", "\002,\002", "\002)", "#",  '\\', "(", ")" }; /* Note: override in SplicePossibleUser() */
struct MODE FMeta = {"\n\002#\002", "\n", "\001",  "\001",      "\n",    "#",  '\\', "(", ")" };
struct MODE KUser = {"",            "",   "(",     ",",         ")",     "#",  0,    "(", ")" };
struct MODE KMeta = {"\n#\002",     "\n", "\001",  "\001",      "\n",    "#",  0,    "",  ""  }; 
struct MODE Tex   = {"\\",          "",   "{",     "}{",        "}",     "#",  '@',  "{", "}" };
...
/* Special Fortran mode where macro name and arguments might be separated
   by one or more blanks when calling macro (but not at macro definition) */
int fortran_mode = 0;
...
void usage(void) {
  ...
  fprintf(stderr,"            [-n] [-C | -F | -f | -T | -H | -X | -P | -U ... [-M ...]] [+c<n> str1 str2]\n");
  ...
  fprintf(stderr," -f : Free-format Fortran compatible mode (includes -n, +c, +s, ...)\n");
  fprintf(stderr," -F : Fixed-format Fortran compatible mode (includes -n, +c, +s, ...)\n");
...
{
  /* 
   * Modification: get rid of delimiter characters if some are left in the output.
   * This can happen for user macros without arguments that are evalated in front 
   ...
   *
   * will be replaced by 
   * function(arg1^B,^Barg2) if the argument separator is defined as "\002,\002"
   * as is currently the case in fortran mode
   *
   * The fix below will not catch all possible delimiters but it is safe and 
   * should be enough in most cases
   *
   * A better fix if one has time to implement it would be to detect when a macro
   * definition has no arguments and not process the "arguments" during a macro 
   * call in this case. 
   * 
   */	
  if (c==2) return;
...
#define DEPRECATED_WARNING fprintf(stderr, "gpp: warning: deprecated option `%s'; use `-%s' instead\n", *arg, *arg)
    ...
    case 'C':
      ...
      break;
    case 'f':  
      /* free-format Fortran mode */
      ishelp|=ismode|hasmeta|usrmode; ismode=1;
      S->User=FUser; S->Meta=FMeta;
      S->preservelf=1;
      fortran_mode=1;
      add_comment(S,"ccc",my_strdup("\\\n"),my_strdup(""),0,0);
      add_comment(S,"ccc",my_strdup("\n#\n"),my_strdup(""),0,0);
      ...
      add_comment(S,"Sss",my_strdup("'"),my_strdup("'"),'\\','\n');
      add_comment(S,"Sss",my_strdup("\""),my_strdup("\""),'\\','\n');
      add_comment(S,"iis",my_strdup("!"),my_strdup("\n"),0,0);
      break;
    case 'F':  
      /* fixed-format Fortran mode */
      ishelp|=ismode|hasmeta|usrmode; ismode=1;
      S->User=FUser; S->Meta=FMeta;
      S->preservelf=1;
      fortran_mode=1;
...
int SplicePossibleUser(int *idstart,int *idend,int *sh_end,int *lg_end,
		       ...
		       int *id,int cmtmode,int ftnmode)
{
  int match,k,pos;
  ...
  *sh_end=*idend;
  ...
  /* Dirty hack: replaced 
     ...
     so that in Fortran mode "#define macro ( with , arguments )" works
     fortran programers like to do this sort of things
  */
  match=matchSequence(ftnmode?"\002(\002":(S->User.mArgS),&pos);
...
int findMetaArgs(int start,int *p1b,int *p1e,int *p2b,int *p2e,int *endm,int *argc,int *argb,int *arge)
{
  ...
  *p1b=pos;
  ...
  if ((*argc)&&SplicePossibleUser(&pos,p1e,&hyp_end1,&hyp_end2,
                                  argb,arge,argc,0,NULL,FLAG_META,0)) {
    *p1b=pos;
...
void SetStandardMode(struct SPECS *P,const char *opt) 
{
  ...
  if (!strcmp(opt,"C")||!strcmp(opt,"cpp")) {
  ...
  else if (!strcmp(opt,"Fortran")||!strcmp(opt,"fortran")) {
    S->User=FUser; S->Meta=FMeta;
    S->preservelf=1;
    fortran_mode=1;
    add_comment(S,"ccc",my_strdup("\\\n"),my_strdup(""),0,0);
    ...
    add_comment(S,"sss",my_strdup("'"),my_strdup("'"),'\\','\n');
    add_comment(S,"sss",my_strdup("\""),my_strdup("\""),'\\','\n');
    add_comment(S,"sss",my_strdup("\n*"),my_strdup("\n"),0,0);
    add_comment(S,"sss",my_strdup("\nc"),my_strdup("\n"),0,0);
    add_comment(S,"sss",my_strdup("\nC"),my_strdup("\n"),0,0);
    add_comment(S,"iss",my_strdup("!"),my_strdup("\n"),0,0);
  }
...
int ParsePossibleUser(void)
{
  ...
  if (!SplicePossibleUser(&idstart,&idend,&sh_end,&lg_end,
                          argb,arge,&argc,1,&id,FLAG_USER,fortran_mode))
    return -1;
  ...
  if (macros[id].nnamedargs==-2) { /* defined(...) macro for arithmetic */
    char *s,*t;
    if (argc!=1)
      bug("defined(...) macro requires an argument");
